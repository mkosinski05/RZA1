...
extern QueueHandle_t g_ceu_queue;
...
uint16_t video_size = (VIDEO_BUFFER_STRIDE * VIDEO_BUFFER_HEIGHT) * VIDEO_BUFFER_NUM;
#endif

static void camera_task (void *parameters)
{
	uint8_t tmp, ceu_mode = 1;

	while (1) {

		// Check if Queue has been created yet
		if ( g_ceu_queue != NULL )
			// Do not wait for queue
			if ( xQueueReceive ( g_ceu_queue, &tmp, 0) )
					ceu_mode = tmp; // change mode is we receive something in queue
